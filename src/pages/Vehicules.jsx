...
import { getVehicles, getTrashVehicles, saveVehicles, saveTrashVehicles, getHistory, saveHistory } from '../database';
...
export default function Vehicules({ user, onNavigate, onLogout }) {
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return vehicles.filter((vehicle) => vehicle.nom.toLowerCase().includes(value));
  }, [search, vehicles]);
  ...
  function handleSubmit(event) {
    ...
    const vehicleName = form.nom.trim() || 'Nouveau véhicule';
    ...
    addToHistory(`Ajouté: ${vehicleName}`, vehicleName);
  ...
  function handleMoveToTrash(vehicleId) {
    ...
    if (user?.role === 'Admin') {
      ...
      const currentUserHistory = getHistory().filter(entry => entry.vehicle === vehicleToTrash.nom);
      const isOwnVehicle = currentUserHistory.some(entry => entry.user === user?.nom);
  ...
  return (
    <div className="dashboard-shell vehicles-shell">
      <UserSidebar user={user} onNavigate={onNavigate} onLogout={onLogout} activePage="vehicules" />
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (user?.role === 'Admin') {
                            ...
                            // Vérifier si le véhicule appartient à l'utilisateur actuel
                            const currentUserHistory = getHistory().filter(entry => entry.vehicle === vehicle.nom);
                            const isOwnVehicle = currentUserHistory.some(entry => entry.user === user?.nom);
                            
                            if (!isOwnVehicle) {
                              alert('Vous ne pouvez modifier que vos propres véhicules.');
                              return;
                            }
                          }
                          
                          const newEtat = vehicle.etat === 'Bon' ? 'En panne' : 
                                         vehicle.etat === 'En panne' ? 'Entretien' : 'Bon';
                        ...
                        style={{
                          ...
                          borderRadius: '4px',
                          background: vehicle.etat === 'Bon' ? '#f0fdf4' : 
                                     vehicle.etat === 'En panne' ? '#fef2f2' : '#fffbeb',
                          cursor: 'pointer',
                          fontSize: '12px',
                          fontWeight: 'bold'
                        }}
          ...
          <div className="modal-form-grid">
            ...
            <div className="form-field">
              ...
              <select
                ...
                onChange={(event) => handleChange('disponibilite', event.target.value)}
              >
