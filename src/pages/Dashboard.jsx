...
export default function Dashboard({ user, onNavigate, onLogout }) {
  ...
  const statCards = useMemo(() => {
    ...
    const enUtilisation = vehicles.filter(v => v.disponibilite === 'Disponible').length + 
                         materials.filter(m => m.disponibilite === 'Disponible').length;
    const horsService = vehicles.filter(v => v.etat !== 'Bon').length + 
  ...
  const activities = useMemo(() => {
    return history.slice(0, 5).map(entry => ({
      date: new Date(entry.date).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: '2-digit' }),
      action: entry.vehicle,
      etat: entry.action.includes('État') ? entry.action.split(' -> ')[1] || 'Bon' : 'Bon',
      disponibilite: entry.action.includes('Disponibilité') ? 
        (entry.action.includes('Disponible') ? 'Disponible' : 'Non disponible') : 
        (entry.action.includes('Ajouté') ? 'Disponible' : 'Non disponible')
    }));
  }, [history]);

  return (
    <div className="dashboard-shell vehicles-shell">
      <UserSidebar user={user} onNavigate={onNavigate} onLogout={onLogout} activePage="dashboard" />
      ...
      <main className="dashboard-main vehicles-main">
        ...
        <section className="dashboard-activity-card">
          <div className="dashboard-card-header">
            ...
            <Button variant="primary" onClick={() => onNavigate('vehicules')}>
              <span aria-hidden="true" className="plus-icon">+</span>
              Véhicules
            </Button>
          </div>
            ...
            <tbody>
              {activities.map((activity) => (
                <tr key={`${activity.date}-${activity.action}`}>
                  ...
                  <td>{activity.etat}</td>
                  <td><StatusBadge value={activity.disponibilite} /></td>
                  <td>
                    <button type="button" className="row-dropdown" aria-label={`Options ${activity.action}`}>
                      <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        <section className="dashboard-chart-card" aria-label="Diagramme de cas d'utilisation FleetManager">
          <h2 style={{ fontSize: '1.2rem', marginBottom: '1.5rem', color: '#333', textAlign: 'center' }}>Diagramme de cas d'utilisation - FleetManager</h2>
          ...
          <div style={{ 
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.8rem', color: '#666' }}>Administrateur</div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.8rem', color: '#666' }}>Utilisateur</div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.7rem', color: '#666' }}>Utilisation</div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.7rem', color: '#666' }}>Approuver/Refuser</div>
            </div>

            {/* Lignes de connexion (simplifiées) */}
            <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
              {/* Lignes de l'Administrateur au système */}
              <line x1="130" y1="90" x2="350" y2="200" stroke="#28c76f" strokeWidth="2" opacity="0.3"/>
              <line x1="130" y1="90" x2="250" y2="140" stroke="#28c76f" strokeWidth="2" opacity="0.3"/>
              <line x1="130" y1="90" x2="450" y2="140" stroke="#28c76f" strokeWidth="2" opacity="0.3"/>
              <line x1="130" y1="90" x2="350" y2="280" stroke="#28c76f" strokeWidth="2" opacity="0.3"/>
              <line x1="130" y1="90" x2="250" y2="240" stroke="#28c76f" strokeWidth="2" opacity="0.3"/>
              <line x1="130" y1="90" x2="450" y2="240" stroke="#28c76f" strokeWidth="2" opacity="0.3"/>
              <line x1="130" y1="90" x2="150" y2="200" stroke="#28c76f" strokeWidth="2" opacity="0.3"/>
              
              {/* Lignes de l'Utilisateur au système */}
              <line x1="570" y1="90" x2="350" y2="200" stroke="#ffcc4d" strokeWidth="2" opacity="0.3"/>
              <line x1="570" y1="90" x2="550" y2="200" stroke="#ffcc4d" strokeWidth="2" opacity="0.3"/>
            </svg>
          </div>

          {/* Légende */}
          <div style={{ marginTop: '1.5rem', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '4px' }}>
            <h3 style={{ fontSize: '1rem', marginBottom: '0.8rem', color: '#333' }}>Légende</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem', fontSize: '0.8rem', color: '#666' }}>
              <div><strong style={{ color: '#28c76f' }}>Administrateur:</strong> Gestion complète du système</div>
              <div><strong style={{ color: '#ffcc4d' }}>Utilisateur:</strong> Consultation et demandes</div>
              <div><strong>Actions principales:</strong> Consulter, Ajouter, Modifier, Supprimer</div>
              <div><strong>Actions avancées:</strong> Maintenance, Demandes, Approbations</div>
            </div>
          </div>
        </section>
      </main>
