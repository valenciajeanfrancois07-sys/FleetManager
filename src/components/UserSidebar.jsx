...
export default function UserSidebar({ user, onNavigate, onLogout, activePage }) {
      ...
      <nav className="sidebar-nav" aria-label="Menu principal">
        {menuItems.map((item) => (
          <button 
            type="button" 
            className={`nav-link ${
              activePage === 'vehicules' && item.label.toLowerCase() === 'véhicules' ? 'active' :
              activePage === 'materiels' && item.label.toLowerCase() === 'matériels' ? 'active' :
              activePage === 'historiques' && item.label.toLowerCase() === 'historique' ? 'active' :
              activePage === item.label.toLowerCase() ? 'active' : ''
            }`} 
            key={item.label}
            onClick={() => onNavigate && onNavigate(
              item.label.toLowerCase() === 'véhicules' ? 'vehicules' :
              item.label.toLowerCase() === 'matériels' ? 'materiels' :
              item.label.toLowerCase() === 'historique' ? 'historiques' :
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            >
              <svg viewBox="0 0 24 24" aria-hidden="true" style={{ width: '20px', height: '20px' }}>
                <path d="M19 12h-6v8H6v-8H0l12-12 12 12zm-6 0h-2v6h2v-6z" fill="currentColor"/>
              </svg>
            </button>
          </div>
